...
#function for yaw calcluation
def calculate_yaw(latitude, longitude, aruco_lat, aruco_lon):
    # Calculate desired yaw
    desired_yaw = np.arctan2(aruco_lat - latitude, aruco_lon - longitude)
    desired_yaw_degrees = np.degrees(desired_yaw)
    print("Desired Yaw:", desired_yaw_degrees)  # yaw in terms of degrees
    return desired_yaw_degrees
...
velocity_y = 0
velocity_z = 0
speed = 0.3 # landing speed in m/s

MARKER_SIZE = 10  # centimeters

#function to calculate the distance between drone and aruco marker
...
#function to set landing speed to 30 cm per second
def set_landing_speed(speed):
    ...
    aTargetAltitude = 2  # Define the value of aTargetAltitude
...
# function to land the vehicle 
    #vehicle should only when "L" key is pressed
    #set the landing speed to 0.5m/s
